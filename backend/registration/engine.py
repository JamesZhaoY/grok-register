...
from backend.mailbox import vmail as vmail_provider
...
# 注册任务上限：Web 接口、任务协调器与引擎共用，避免各处硬编码后漂移。
MAX_REGISTER_WORKERS = 10
MAX_REGISTER_COUNT = 100000
...
DEFAULT_CONFIG = {
    ...
    "cloudmail_password": "",
    "vmail_api_base": "https://mail.22y.uk",
    "vmail_api_key": "",
    "vmail_default_domain": "",
    "vmail_expires_in": 86400,
    "cloudflare_api_base": "",
    "cloudflare_api_key": "",
...
def add_sso_to_cpa(raw_token, email="", log_callback=None, result_out=None) -> bool:
    ...
    # grok_web 模式:SSO 直写,无需 OAuth 换 token
    g2a_provider = str(config.get("grok2api_provider", "grok_web") or "grok_web").strip().lower()
    if g2a_provider == "grok_web" and g2a_dir and not auth_dir and not remote_url:
        _cpa_log("grok_web 模式:SSO 直写,跳过 OAuth token 交换")
        gpath = _s2cpa.write_grok2api_web_auth(_s2cpa.Path(g2a_dir), sso, email=email)
        _cpa_log(f"已写入 Grok2API(grok_web) {gpath}")
        _set_result(
            status="success",
            auth_info=[f"Grok2API(grok_web): {gpath}"],
            auth_path=str(gpath),
            cpa_auth_path="",
            grok2api_auth_path=str(gpath),
            error="",
        )
        return True
                ...
                if g2a_remote_configured and g2a_auto_import:
                    try:
                        ...
                        with _grok2api.Grok2APIClient.from_config(config) as client:
                            g2a_provider = str(config.get("grok2api_provider", "grok_web") or "grok_web").strip().lower()
                            remote_result = client.import_auth_file(gpath, provider=g2a_provider)
...
def get_vmail_api_base():
    return str(os.environ.get("VMAIL_API_BASE") or config.get("vmail_api_base", "") or "").strip().rstrip("/")


def get_vmail_api_key():
    return str(os.environ.get("VMAIL_API_KEY") or config.get("vmail_api_key", "") or "").strip()


def get_vmail_default_domain():
    domain = str(config.get("vmail_default_domain", "") or "").strip()
    if domain:
        return domain
    raw_domains = str(config.get("defaultDomains", "") or "")
    domains = [item.strip() for item in re.split(r"[,，\s]+", raw_domains) if item.strip()]
...
def get_vmail_expires_in():
    try:
        return max(0, int(config.get("vmail_expires_in", 86400) or 0))
    except (TypeError, ValueError):
        return 86400


def vmail_get_email_and_token():
    return vmail_provider.create_mailbox(
        http_post,
        get_vmail_api_base(),
        get_vmail_api_key(),
        domain=get_vmail_default_domain(),
        local_part=generate_username(10),
        expires_in=get_vmail_expires_in(),
    )


def vmail_get_oai_code(
    dev_token,
    email,
    timeout=180,
    poll_interval=3,
    ...
    return vmail_provider.wait_for_code(
        http_get,
        get_vmail_api_base(),
        get_vmail_api_key(),
        dev_token,
...
def get_email_and_token(api_key=None):
    ...
    if provider == "vmail":
        return vmail_get_email_and_token()
...
def run_registration(count):
    ...
    workers = max(
        1,
        min(
            int(config.get("register_workers", 1) or 1),
            MAX_REGISTER_WORKERS,
            int(count or 1),
        ),
    )
