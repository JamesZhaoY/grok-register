...
XAI_SIGNUP_URL = "https://accounts.x.ai/sign-up?redirect=cloud-console"
...
def check_email_api(provider: str, config: dict, http_get: Callable, http_post: Callable) -> CheckResult:
    ...
    try:
        ...
        if provider == "vmail":
            key = str(config.get("vmail_api_key", "") or "").strip()
            if not key:
                return "邮箱API", False, "VMail 需配置 API Key"
            base = str(config.get("vmail_api_base", "") or "https://mail.22y.uk").strip().rstrip("/")
            if not base:
                base = "https://mail.22y.uk"
            ...
            # 用不存在的 mailbox 探测鉴权；401=密钥无效，404=密钥有效
            resp = http_get(
                f"{api_root}/mailboxes/__connectivity_probe__",
                headers={"X-API-Key": key, "Accept": "application/json"},
                timeout=12,
            )
            if resp.status_code in (401, 403):
                return "邮箱API", False, f"VMail API Key 无效 HTTP {resp.status_code}"
            if resp.status_code >= 500:
                return "邮箱API", False, f"VMail 服务异常 HTTP {resp.status_code}"
            return "邮箱API", True, f"VMail 可达 HTTP {resp.status_code}"
