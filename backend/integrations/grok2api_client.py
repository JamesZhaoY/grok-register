...
class Grok2APIClient:
    ...
    IMPORT_WEB_PATH = "/api/admin/v1/accounts/web/import"
    CONFIG_KEYS = (
        "grok2api_remote_url",
    ...
    def import_auth_file(self, file_path: str | Path, provider: str = "grok_build") -> Dict[str, Any]:
        """自动登录并将一个 JSON 导入远程管理端。

        provider 决定导入路径:
          - "grok_build" -> /accounts/import
          - "grok_web"   -> /accounts/web/import
        """
        path, content = self._load_auth_document(file_path)
        token = self.login()
        import_path = (
            self.IMPORT_WEB_PATH
            if str(provider or "").strip().lower() == "grok_web"
            else self.IMPORT_PATH
        )
        multipart = CurlMime()
        multipart.addpart(
            name="files",
