...
def grok2api_web_auth_filename(entry: dict, email: str = "") -> str:
    """Grok2API grok_web 导入文件名。"""
    ident = (
        str(email or "").strip()
        or str(entry.get("email") or "").strip()
        or str(entry.get("name") or "").strip()
    ...
    safe = _safe_email_for_filename(ident)
    return f"g2a-web-{safe}.json"


def sso_to_grok2api_web_account(sso: str, email: str = "") -> dict:
    """SSO cookie → Grok2API grok_web 导入账号条目(无需 OAuth 换 token)。

    对齐 grok2api:
      - backend/internal/infra/provider/web/import.go importEntry
      - provider: grok_web, auth_type: SSO(sso_token 直接入库)
    """
    sso_val = str(sso or "").strip()
    email_val = str(email or "").strip()
    name = email_val or "Grok Web account"

    payload = decode_jwt_payload(sso_val)
    user_id = (
        str(payload.get("sub") or "").strip()
        or str(payload.get("principal_id") or "").strip()
...
def write_grok2api_web_auth(auth_dir: Path, sso: str, email: str = "") -> Path:
    """写出 Grok2API 可导入的 grok_web auth(accounts[] 包装,SSO 直写)。"""
    auth_dir.mkdir(parents=True, exist_ok=True)
    account = sso_to_grok2api_web_account(sso, email=email)
    path = auth_dir / grok2api_web_auth_filename(account, email=email)
    document = {"accounts": [account]}
    tmp = path.with_suffix(path.suffix + ".tmp")
    tmp.write_text(
        json.dumps(document, ensure_ascii=False, indent=2) + "\n",
