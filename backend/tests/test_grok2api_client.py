...
class Grok2APIClientTests(unittest.TestCase):
    ...
    def test_import_web_uses_web_path(self):
        import_response = FakeResponse(
            lines=[
                b"event: complete",
                b'data: {"created":1,"updated":0,"synced":1}',
        ...
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "g2a-web-fixture.json"
            path.write_text(json.dumps({"provider": "grok_web"}), encoding="utf-8")
            result = client.import_auth_file(path, provider="grok_web")
        self.assertEqual(result["created"], 1)
        self.assertEqual(
            session.calls[1][0],
