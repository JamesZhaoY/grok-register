    ...
    for key in CONFIG_PUBLIC_KEYS:
        ...
        if key in (
            ...
            if key == "register_count":
                value = max(1, min(value, gr.MAX_REGISTER_COUNT))
            elif key == "register_workers":
                value = max(1, min(value, gr.MAX_REGISTER_WORKERS))
            ...
            value = mode
        elif key == "grok2api_provider":
            value = str(value or "").strip().lower()
            if value not in ("grok_build", "grok_web"):
                value = "grok_web"
            value = value
        elif key in (
            "proxy",
    ...
    def api_account_grok2api_import(account_id: int) -> Dict[str, Any]:
        ...
        try:
            with Grok2APIClient.from_config(gr.config) as client:
                g2a_provider = str(gr.config.get("grok2api_provider", "grok_web") or "grok_web").strip().lower()
                result = client.import_auth_file(path, provider=g2a_provider)
