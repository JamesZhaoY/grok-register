...
LABEL org.opencontainers.image.title="grok-register" \
      org.opencontainers.image.description="Grok 账号注册管理台（FastAPI + React + Camoufox）" \
      org.opencontainers.image.source="https://github.com/kaibush/grok-register" \
...
ENV PATH=/opt/venv/bin:$PATH \
    ...
    GROK_CONFIG_TEMPLATE=/app/config.example.json \
    GROK_LOG_KEEP=20 \
    GROK_FORCE_HEADED=1
