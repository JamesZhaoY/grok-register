...
const PROVIDERS = [
  ...
  },
  {
    value: "vmail",
    label: "VMail 临时邮箱",
    description: "兼容 vmail.dev / mail.22y.uk Open API，需要 API Key。",
  },
];
      ...
      <div className="grid items-start gap-4 lg:grid-cols-[220px_minmax(0,1fr)]">
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>选择账号类型:grok_web(SSO 直写)或 grok_build(OAuth token)。</CardDescription>
              </CardHeader>
            ...
            <Card>
              ...
              <CardContent className="grid gap-4">
                <ConfigField {...fieldState} label="账号类型" field="grok2api_provider" placeholder="grok_web | grok_build" helper="grok_web=SSO 直写(默认);grok_build=OAuth token" />
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 sm:grid-cols-2">
            ...
            {selectedProvider.value === "vmail" ? (
              <>
                <ConfigField
                  {...fieldState}
                  label="API Base"
                  field="vmail_api_base"
                  helper="例如 https://mail.22y.uk，不要附加 /api-docs"
                />
                <ConfigField
                  {...fieldState}
                  label="API Key"
                  field="vmail_api_key"
                  type="password"
                  helper="在站点 /api-docs 页面创建，请求头使用 X-API-Key"
                />
                <ConfigField
                  {...fieldState}
                  label="固定收信域名"
                  field="vmail_default_domain"
                  helper="留空则用 defaultDomains 第一个，或由服务端默认域名分配"
                />
                <ConfigField
                  {...fieldState}
                  label="邮箱有效期(秒)"
                  field="vmail_expires_in"
                  type="number"
                  helper="对应 expiresIn，默认 86400（24 小时），0 表示不传"
                />
              </>
            ) : null}
